...
3. For each event in `pending-events.json`:
   - For each character listed in `event.characters`:
     ...
     - Read the character's `soul.md` and `personality.md` from `worlds/billings-montana/characters/`. Check if the proposed activity strongly contradicts their personality → **warning** only (never a hard block).
...
- `absent` health_status is a hard block — an absent character cannot participate in local events.
